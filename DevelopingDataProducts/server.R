...
shinyServer(
  function(input, output) {
    ...
    output$plots <- renderPlot({
      ...
      points(input$hp, predicted_mpg(), col='blue', cex=3.3, pch=21)                 
      # (1, 2)
      with(mtcars, plot(cyl, mpg,
                        xlab='Number of cylinders',
                        ylab='MPG',
                        main='MPG vs cylinders'))
      points(as.numeric(input$cyl), predicted_mpg(), col='red', cex=3.3, pch=22)  
      # (1, 3)
      with(mtcars, plot(wt, mpg,
                        xlab='Weight (lb/1000)',
                        ylab='MPG',
                        main='MPG vs weight'))
      points(adjusted_weight(), predicted_mpg(), col='green', cex=3.3, pch=23)  
    })
  }
)